	...
	public boolean isValid(EvaluationContext context) {
		...
		boolean lava =
			currentBlock.hasTag(BlockTags.IS_LAVA) ||
				topBlock.hasTag(BlockTags.IS_LAVA) ||
				belowBlock.hasTag(BlockTags.IS_LAVA) ||
				below2Block.hasTag(BlockTags.IS_LAVA);

		if (lava) return false;

		boolean water =
			currentBlock.hasTag(BlockTags.IS_WATER) ||
				topBlock.hasTag(BlockTags.IS_WATER);

		if (water) return false;

		boolean headClear =
			!currentBlock.isCollidable() ||
				currentBlock.getLogic() instanceof BlockLogicFlower;

		boolean bodyClear = !topBlock.isCollidable();

		boolean solidFloor =
			belowBlock.isCollidable() &&
				!(belowBlock.getLogic() instanceof BlockLogicFlower);

		boolean solidFloor2 = below2Block.isCollidable() && !(below2Block.getLogic() instanceof BlockLogicFlower);

		if (!solidFloor) {
			return headClear && bodyClear && solidFloor2;
		} else {
			return headClear && bodyClear;
		}
	}
}
